...
function parseCliOptions(): CliOptions {
    ...
    const fileArg = getOptionValue(argv, 'file') ?? positional[0] ?? 'planilla descartable uti.xlsx'
...
function parseDescartables(filePath: string, requestedSheet?: string): string[] {
    ...
    const headerRowIndex = rows.findIndex((row) =>
        (row ?? []).some((cell) => normalizeHeader(cell) === 'DESCARTABLE')
    )
    if (headerRowIndex < 0) {
        throw new Error('No se encontró encabezado "DESCARTABLE" en la planilla.')
    }

    const markerColumns = (rows[headerRowIndex] ?? [])
        .map((cell, idx) => (normalizeHeader(cell) === 'DESCARTABLE' ? idx : -1))
        .filter((idx) => idx >= 0)
...
async function main() {
    ...
    const descartables = parseDescartables(options.filePath, options.sheetName)

    console.log(`Archivo: ${options.filePath}`)
    console.log(`Descartables únicos detectados: ${descartables.length}`)
    ...
    for (const nombre of descartables) {
        await prisma.catalogoDescartableUti.upsert({
            where: { nombre },
            create: {
                nombre,
                estado: 'A',
    ...
    console.log('Importación de descartables UTI finalizada correctamente.')

    await prisma.$disconnect()
}

main().catch((error) => {
    console.error('Error en importación de descartables UTI:', error)
    process.exitCode = 1
})
