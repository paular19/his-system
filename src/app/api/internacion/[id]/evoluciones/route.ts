...
import { CrearEvolucionSchema } from '@/modules/internacion/schemas'
...
export async function POST(req: NextRequest, { params }: RouteParams) {
    try {
        ...
        const validado = CrearEvolucionSchema.parse({ ...body, ingresoId })

        const ip = req.headers.get('x-forwarded-for') ?? req.headers.get('x-real-ip') ?? undefined
        const evolucion = await service.crearEvolucion(validado, usuario.codigoUsuario, ip ?? undefined)
