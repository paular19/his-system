...
import { esRolHIS, type RolHIS } from './rbac'
import {
...
function resolverRolFallbackPorEmail(email: string | undefined): RolHIS | null {
  return resolverRolRegistradoPorEmail(email)
}
...
function resolverRolDesdeClaims(claims: unknown): RolHIS {
  ...
  if (rolForzado) return rolForzado

  const metadata = resolverMetadataDesdeClaims(claims)
  const rolMetadata = asString(metadata.rol)

  if (esRolHIS(rolMetadata)) {
    return rolMetadata
  }

  const rolFallback = resolverRolFallbackPorEmail(resolverEmailDesdeClaims(claims))
  if (rolFallback) return rolFallback
...
function resolverRolDesdeMetadata(user: Awaited<ReturnType<typeof currentUser>>): RolHIS {
  ...
  const rolForzado = resolverRolForzadoPorEmail(emailPrincipal)
  if (rolForzado) return rolForzado
  ...
  const rolFallback = resolverRolFallbackPorEmail(emailPrincipal)
  if (rolFallback) return rolFallback
...
export async function getUsuarioSesionLectura(): Promise<UsuarioSesion> {
  ...
  let email = resolverEmailDesdeClaims(sessionClaims)
  let nombre = resolverNombreDesdeClaims(sessionClaims)
  let codigoUsuario = resolverCodigoUsuarioDesdeClaims(sessionClaims, userId)
  let rol: RolHIS
  ...
  try {
    ...
    rol = resolverRolDesdeMetadata(user)
    email = user.emailAddresses[0]?.emailAddress ?? ''
    nombre = `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim()
    codigoUsuario =
      ((user.publicMetadata?.codigoUsuario as string | undefined) ?? userId).slice(0, 10)
  }
