...
/** Cambia el medico firmante (cabecera) de una orden ya generada. */
export async function actualizarFirmanteOrdenAction(
  puestoNumero: number,
  numero: number,
  ...
  if (!Number.isFinite(profesionalId) || profesionalId <= 0) {
    return { error: 'Seleccioná un profesional válido' }
  }

  try {
    const orden = await prisma.orden.findUnique({
      where: { puestoNumero_numero: { puestoNumero, numero } },
      select: { ingresoId: true, estado: true },
    })

    if (!orden) return { error: 'La orden no existe' }
    if ((orden.estado ?? '').trim().toUpperCase() === 'X') {
      return { error: 'La orden está anulada' }
    }
    ...
    if (!profesional) return { error: 'El profesional no existe' }
    ...
    console.error('[ORDEN] Error al actualizar firmante:', err)
    return { error: 'Error al guardar el médico firmante' }
  }
}
